...
const SearchEList = (props) => {
  ...
  //USE ROUTER
  const {track, playlist, artist, album } = props;
        ...
        <div className={cx(style.child, style.container)}>
          ...
          {displayedTracks.map((unit) => (
            <div className={cx(style.card, style.child)} key={unit.id}>
              ...
              />
              <p style={{ fontWeight: "bold" }} className={style.truncate}>
                {unit.name}
              </p>
      ...
      {album.length > 0 ? (
        <div className={cx(style.container, style.child)}>
          ...
          {displayedAlbums.map((unit) => (
            <div
              className={cx(style.card, style.child)}
              key={unit.id}
              onClick={() => openAlbum(unit.id)}
            >
              <img
      ...
      {artist.length > 0 ? (
        <div className={cx(style.container, style.child)}>
          ...
          {displayedArtists.map((unit) => (
            <div
              className={cx(style.card_artist, style.child)}
              key={unit.id}
              onClick={() => openArtist(unit.id)}
            >
              <img
...
export default withRouter(SearchEList);